...
class WeatherViewController: UIViewController {
    ...
    fileprivate func computeWeatherDetailsBeforeRendering(weatherModel: WeatherModel?, ifCurrentLocation: Bool) {
        DispatchQueue.main.async { [weak self] in
            ...
            strongSelf.labelTemperature.text = "\(Int(weatherModel?.current.tempC ?? 0.0)) °C"
            ...
            LocalDataManager.weatherCollection.append(WeatherLocalModel(cityName: locationName, weatherCondition: weatherModel?.current.condition?.text ?? "", temperature: String(weatherModel?.current.tempC?.rounded() ?? 0.0), imageCode: weatherModel?.current.condition?.code ?? 0))
        }
    }
    ...
    @IBAction private func didSelectSegmentControl(_ sender: UISegmentedControl) {
        ...
        case 0:
            self.labelTemperature.text = "\(Int(weather.current.tempC ?? 0.0)) °C"
        case 1:
            self.labelTemperature.text = "\(Int(weather.current.tempF ?? 0.0)) °F"
        default:
            break
        }
    }
